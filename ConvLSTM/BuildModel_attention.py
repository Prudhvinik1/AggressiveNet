...
from keras.layers import Dense, Flatten, Dropout, ZeroPadding3D, ConvLSTM2D, Reshape, BatchNormalization, Activation, RepeatVector, Permute, Multiply
...
def build(size, seq_len , learning_rate ,
    ...
    pool = MaxPooling2D(pool_size=(2, 2))(lstm)
    flat = Flatten()(pool)
    flat = BatchNormalization()(flat)
    flat = Dropout(dropout)(flat)

    #apply Simple attention To The Context Vector In The End
    attention = Dense(1,activation='tanh')(flat)
    attention = Activation('softmax')(attention)
    attention_vecs = Multiply()([flat,attention])

    linear = Dense(1000)(attention_vecs)
    ...
    parallel_model.compile(optimizer=optimizer, loss=loss_func,metrics=['acc'])
